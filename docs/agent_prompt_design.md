...
is deterministic (`app/agents/llm.py::call_llm_json`) and the mock LLM can
implement the identical contract for offline/demo use.
...
`app/agents/llm.py::MockChatModel` implements the exact same JSON contract
per prompt shape (matched by a distinctive substring in each prompt, e.g.
`"return a json object with keys: is_suspicious"`), using a keyword→severity
heuristic (`SEVERITY_KEYWORDS`). This lets the whole four-agent chain run
end-to-end, deterministically, without any API key — useful for grading,
CI, and offline demos — while keeping the code path identical to the real
LLM integration.
